...
class AppUserControllerTest {
  ...
  public void testGetUserById() throws Exception {
    ...
    // When and Then
    mockMvc
        .perform(get(this.baseUrl + "/users/{id}", userId).accept(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.flag").value(true))
        .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
        .andExpect(jsonPath("$.message").value("Success"))
  ...
  void testFindUserByIdNotFound() throws Exception {
    ...
    given(this.appUserService.findById(userId))
        .willThrow(new ObjectNotFoundException("user", userId));

    // When and then
    this.mockMvc
        .perform(get(this.baseUrl + "/users/5").accept(MediaType.APPLICATION_JSON))
  ...
  public void testCreateUserSuccess() throws Exception {
    ...
    given(this.appUserService.addNewUser(Mockito.any(NewAppUserRequest.class)))
  ...
  public void testDeleteUser() throws Exception {
    ...
    mockMvc
        .perform(
            delete(this.baseUrl + "/users/{userId}", userId).accept((MediaType.APPLICATION_JSON)))
        .andExpect(jsonPath("$.flag").value(true))
        .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
  ...
  public void testDeleteUserErrorWithNonExistentId() throws Exception {
    ...
    doThrow(new ObjectNotFoundException("user", userId))
        .when(this.appUserService)
