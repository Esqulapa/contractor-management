...
public class ContractorBillingController {
  ...
  public Result getContractorBillings() {
    List<ContractorBillingDTO> contractorBillingDTOs =
        contractorBillingService.findAll().stream()
  ...
  public Result getContractorBillingById(@PathVariable("contractorId") Integer id) {
    return new Result(
        true, StatusCode.SUCCESS, "Success", contractorBillingService.getContractorBillingById(id));
  }

  @GetMapping("/report")
  public Result getContractorBillingsMonthlyReport(
      @Valid @RequestParam("year") Year year, @RequestParam("month") Month month) {
    return new Result(
        true,
        StatusCode.SUCCESS,
        "Success",
  ...
  @PostMapping
  public Result addContractorBilling(@Valid @RequestBody NewContractorBillingRequest request)
      throws ContractorAlreadyExistInGivenTimeException {
  ...
  public Result updateContractorBillingWorkedHours(
      @PathVariable("contractorBillingId") Integer id,
      @Valid @RequestBody UpdateContractorBillingHoursRequest updateRequest)
      throws SameHoursOrLessThanZeroException {
